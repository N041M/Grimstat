...
/**
 * Whole models with at least one wound each, as the encoding above requires. Both backends read
 * their groups through this, so they solve the same defender.
 */
 ...
 * result. Consecutive faces damaging the same groups form one class, so the classes come out in
 * ascending face order, which is the order the results are resolved in.
 */
