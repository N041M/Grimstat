...
- Saves: invulnerable checked on the unmodified roll, armour on the AP-modified roll, best of both; an unmodified 1 always fails, and a 6 saves only when one of the two saves reaches it; save-roll modifiers capped ±1.
- Save order: under 11e one weapon profile's save rolls are all made first and resolved from the lowest result up, each against whichever allocation group is current when it is reached. This only differs from allocating one wound at a time when the groups differ in which results save them, such as a character with an invulnerable save leading a squad, and the exact backend then solves it as a multinomial over the result classes. 10e allocates one wound at a time. A re-roll of failed saves is judged against the group the results reach first.
...
- **Hazardous**: 11e fails on 1–2 and costs 1 mortal wound (3 if the firing unit is entirely VEHICLE/MONSTER); 10e fails on a 1 and costs 3 mortal wounds whatever the model. Reported as expected self-inflicted mortal wounds, not applied to the attacker's profile.
