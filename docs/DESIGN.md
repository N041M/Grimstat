...
- **Sequence:** Attacks → Hit (unmod 1 fails, unmod 6 = crit) → Wound (S vs T table; unmod 6 = crit) → Save (all of a profile's saves rolled together; invuln vs *unmodified* roll; else AP-modified armour; unmod 1 always fails, no automatic 6) → Damage (lowest save result resolved first, each against the current allocation group) → FNP per wound → Mortal wounds resolved *after* the sequence.
